...
| | |
|---|---|
| Plugin directory | `stop-user-enumeration/` |
| Main file | `stop-user-enumeration/stop-user-enumeration.php` |
| Default branch | `main` |
| WordPress.org slug | `stop-user-enumeration` |
| wp-env ports | dev `8800`, tests `8801` |
| Version locations | plugin header `Version:`, `readme.txt` `Stable tag:` and `STOP_USER_ENUMERATION_PLUGIN_VERSION` in the main file |
...
npm run start                     # wp-env (dev :8800, tests :8801, admin/password)
npm test                          # PHPUnit inside the wp-env tests container
npm test -- --filter Foo          # pass PHPUnit args through
composer run build                # zipped/stop-user-enumeration-free.zip via wp dist-archive
```
